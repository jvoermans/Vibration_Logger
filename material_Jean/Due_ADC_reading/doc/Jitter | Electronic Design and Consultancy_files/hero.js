(function () {
	...
	HeroSlider.prototype.init = function () {
		...
		for (var i = 0; i < this.navigationItems.length; i++) {
			...
			var item = self.navigationItems[i];
			var makeClickHandler = function(item) {
				return function (event) {
					event.preventDefault();
					if (hasClass(item, 'cd-selected'))
						return;
					self.oldSlideIndex = self.newSlideIndex;
					self.newSlideIndex = Array.prototype.indexOf.call(self.navigationItems, item);
					self.newSlide();
			...
			item.addEventListener('click', makeClickHandler(item));
		}
	...
	if (document.getElementsByClassName('js-cd-header__nav').length) {
